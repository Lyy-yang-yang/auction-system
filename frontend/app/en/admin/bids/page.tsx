...
export default function AdminBidsPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">Bid Records</h1>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Bid Time</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {bids.map((bid) => (
              <tr key={bid.id}>
                <td className="px-6 py-4">{new Date(bid.created_at).toLocaleString('en-US')}</td>
            ...
            {bids.length === 0 && (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-gray-500">No bids found</td>
              </tr>
            )}
          </tbody>
        </table>
